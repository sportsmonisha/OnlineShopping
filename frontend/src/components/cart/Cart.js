...
import { decreaseCartItemsQty, increaseCartItemsQty, removeItemFromCart } from "../../slices/cartSlice";
...
export default function Cart() {
    ...
    const increaseQty = (item) => {
        ...
        dispatch(increaseCartItemsQty(item.product))
    }
    const decreaseQty = (item) => {
        const count = item.quantity;
        if(count ===1) return
        dispatch(decreaseCartItemsQty(item.product))
    }
